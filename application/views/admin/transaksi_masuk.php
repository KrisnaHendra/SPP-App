...
<div class="card shadow mb-4">
            <div class="card-header py-3 bg-primary">
                <div class="row">
              <h4 class="m-0 font-weight-bold text-light col-md-4 mr-5 pr-5"><?= $jumlah; ?> Transaksi Masuk</h4><h2 class="col"></h2>
                ...
                <table class="table table-sm table-striped table-hover" id="dataTable" width="100%" cellspacing="0">
                  <thead>
                    <tr class="bg-info text-white">
                      ...
                      <th>Status</th>
                      <th>Tanggal</th>
                      <th width="30px;" class="text-center"><i class="fa fa-edit"></i></th>
                    </tr>
                  </thead>
                  ...
                  <tr>
                        ...
                        <td>
                        <?php if($t['status']==0){ ?>
                        <a href="" class="badge badge-danger" ><i class="fa fa-recycle"></i> Process</a>
                        <?php }else{ ?>
                        <a href="" class="badge badge-info" ><i class="fa fa-check"></i> Success</a>
                        <?php } ?>
                        </td>
                        <td><?= date('d F Y',$t['created_at']) ?></td>
                        <td>
                        <?php if($t['status']==0){ ?>
                        <a href="#modalEdit<?= $t['id_pembayaran'] ?>" class="btn btn-sm btn-outline-warning" style="" data-toggle="modal"><i class="fa fa-edit"></i></a>
                        <?php }else{ ?>
                        <a href="" class="btn btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="Success"><i class="fa fa-check-circle"></i></a>
                        <?php } ?>
                        </td>
                  </tr>
                  <?php endforeach; ?>
...
<div class="modal fade" id="modalEdit<?= $t['id_pembayaran'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <?php if($t['bayar']=='SPP'){?>
      <form action="<?= base_url('admin/ubah_status_spp') ?>" method="POST">
      ...
      <?php } elseif($t['bayar']=='DSP'){?>
      <form action="<?= base_url('admin/ubah_status_dsp') ?>" method="POST">
      <input type="hidden" name="id_pembayaran" value="<?= $t['id_pembayaran'] ?>">
      <input type="hidden" name="nisn" value="<?= $t['nisn'] ?>">
      <input type="hidden" name="jumlah" value="<?= $t['jumlah'] ?>">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
      ...
      <?php } else { ?>
        <form action="<?= base_url('admin/ubah_status_lain') ?>" method="POST">
      <input type="hidden" name="id_pembayaran" value="<?= $t['id_pembayaran'] ?>">
      <input type="hidden" name="nisn" value="<?= $t['nisn'] ?>">
      <input type="hidden" name="jumlah" value="<?= $t['jumlah'] ?>">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
